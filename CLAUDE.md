...
- **ORPC procedures** in `src/router/middleware.ts`: `publicProcedure`, `authProcedure`, `adminProcedure` — each adds auth context
...
- Config in `src/config/` (env, auth, nodemailer)
...
- Auth client in `src/lib/auth.ts` (Better Auth)
- State: Jotai atoms in `src/atoms/`, server state via TanStack Query/ORPC
